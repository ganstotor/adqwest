...
const MainPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDriverCampaigns = async () => {
      ...
      try {
        ...
        const driverCampaignsRef = collection(db, 'driver_campaigns');
        const q = query(
        ...
        const filteredCampaigns = campaignList.filter(Boolean) as DriverCampaign[];
  ...
  const handleOrderBags = () => {
    router.push('/drops/order-bags');
  };

  const handleViewMissions = (campaignId: string) => {
    router.push({ pathname: '/drops/missions', params: { driverCampaignId: campaignId } });
  };

  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>Already have a case? Click “Scan Case”</Text>
      <Text style={styles.title}>Don’t have a case yet? Click “Order Bags”</Text>

      <TouchableOpacity style={[styles.button, { backgroundColor: '#90EE90' }]} onPress={handleScanCase}>
        <Text style={styles.buttonText}>Scan Case</Text>
      </TouchableOpacity>

      <TouchableOpacity style={[styles.button, { backgroundColor: '#FFA500' }]} onPress={handleOrderBags}>
      ...
      {driverCampaigns.map((campaign) => (
        <View key={campaign.id} style={styles.card}>
          ...
          <View style={styles.details}>
            <Text style={styles.text}><Text style={styles.label}>Company:</Text> {campaign.companyName}</Text>
            <Text style={styles.text}><Text style={styles.label}>States:</Text> {campaign.states.join(', ')}</Text>
            <Text style={styles.text}><Text style={styles.label}>Status:</Text> {campaign.status}</Text>
          </View>
          <TouchableOpacity onPress={() => handleViewMissions(campaign.id)} style={styles.missionButton}>
